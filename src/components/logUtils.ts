...
export const columns: LogColumn[] = [
	...
	{ key: 'url', label: 'Endpoint', sortable: true, grow: true },
	{ key: 'status', label: 'Status', sortable: true },
	{ key: 'duration', label: 'Ping', sortable: true },
...
export function parseLogEntry(log: LogEntry, index: number): ParsedLog {
	...
	// Override formatting for server console logs that might have been mangled by older SSE formatters
	if (url.startsWith('[Server Console]') || url.startsWith('[Server Inspect]')) {
		tag = 'server console';
		method = '-';
		finalStatusText = isError ? 'Error' : 'Info';
		statusCode = null;
		
		// Remove the confusing "[API] - " prefix from the message if it exists
		if (message.startsWith('[API] - ')) {
			message = message.substring(8);
			if (message.endsWith(' - success') || message.endsWith(' — success')) {
...
export function getStatusBadgeClass(log: ParsedLog) {
	if (log.isPending) return 'bg-amber-500/15 text-amber-500 border-amber-500/30';
