...
export interface CaptureOptions {
	...
	/** Completely disable interception of server-side fetch calls (useful if Next.js fetch patches conflict). */
...
function captureResponse(res: any, meta: CaptureMeta, opts: CaptureOptions) {
	...
	const maybeStartRewrite = (s: number) => {
		...
		if (isCompressedResponse()) return;
		const ctRaw = typeof res.getHeader === 'function' ? res.getHeader('content-type') : contentType;
		const ct = Array.isArray(ctRaw) ? ctRaw.join(';') : (ctRaw ? String(ctRaw) : '');
...
function captureRequest(req: any, res: any, opts: CaptureOptions) {
	...
	if (path.includes('/log-inspector/')) return;
...
export function installServerCapture(options: CaptureOptions = {}): () => void {
	...
	let uninstallFetch = () => {};
	if (!options.disableServerFetch) {
		uninstallFetch = installServerFetchInterceptor({
